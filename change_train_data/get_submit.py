...
predict_info_path_rf1= write_dir + "rf_pre_0415_info_change_train.csv"
predict_pair_info_path_rf1 =  write_dir + "predict_0415_data"
...
def run_demo(k):
    ...
    userid_list_rf1=[]
    skuid_list_rf1=[]
    pair_info_rf1=csv.reader(open(predict_pair_info_path_rf1))
    for row in pair_info_rf1:
        userid_list_rf1.append(int(row[0]))
        skuid_list_rf1.append(int(row[1]))
    ...
    pd.DataFrame(pre_pair_list,columns=["user_id","sku_id"]).to_csv("submit_change_train_data_rf.csv",index=False)
