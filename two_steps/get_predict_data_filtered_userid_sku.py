...
def get_predict_data(predict_0415_result_path):
...
#------------------------------------------------------------读取文件信息------------------------------------------------#
    ...
    filtered_skuid = set(filtered_data_5days["sku_id"].tolist())
    filtered_skuid = list(filtered_skuid)
    # 构建预测集需要的userid
    filtered_userid_path=dealed_data_dir + "predict_userid.csv"
    filtered_userid_info=pd.read_csv(filtered_userid_path)
    filtered_userid=filtered_userid_info["user_id"].tolist()
...
#---------------------------------------遍历所有userid和skuid，生成4/11当天的数据-----------------------------------------#
    # predict_0410_list=[]
    # f_write=open(predict_0415_result_path,"w+")
    predict_0415_list = []
...
#将预测集合存储
# predict_0415_result_path = dealed_data_dir + "predict_0415_data_2_steps.csv"
# get_predict_data(predict_0415_result_path)
