...
#---------------------------------------遍历所有userid和skuid，生成4/16当天的数据-----------------------------------------#
    ...
    for user_id,sku_id in user_sku_list_zip_set:
        ...
        # print(sales,clicks,add2cars,views)
        sku_click_buy_rate = sales / clicks
        sku_view_buy_rate = sales / views
        sku_add2car_buy_rate = sales / add2cars
        print(sales, clicks, add2cars, views, sku_click_buy_rate, sku_add2car_buy_rate, sku_view_buy_rate)

        # 商品总销量
        temp_list.append(sku_total_sales[sku_id])
        # 用户总动作次数
        temp_list.append(user_total_actions[user_id])
        #转化率
