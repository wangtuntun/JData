...
def format_data(raw_file_path,write_path):
    ...
    for row in raw_data:
        ...
        for i in range(14,64):#接下来的37个统计特征
            row[i]=math.log(1.0 + float(row[i]))
            temp_list.append(row[i])

        #转化率
        temp_list.append(row[64])
        temp_list.append(row[65])
        temp_list.append(row[66])

        #曜日
        # temp_list.extend(one_hot_code(row[67], weekday_range))
        temp_list.extend(row[67])
        #假期
        temp_list.append(row[68])
        # 标签
        label=row[69]
...
raw_path = write_dir + "train_balanced_1vs1_less"
result_path= write_dir + "train_balanced_1vs1_less_formatted_0518"
