...
predict_info_path_rf1= write_dir + "xgb_lv1_less_0518_2.csv"
predict_pair_info_path_rf1 =  write_dir + "add_feature_predict.csv"
...
def run_demo(k):
    ...
    for row in pair_info_rf1:
        userid_list_rf1.append(int(float(row[0])))
        skuid_list_rf1.append(int(float(row[1])))
    ...
    pd.DataFrame(pre_pair_list,columns=["user_id","sku_id"]).to_csv("submit_xgb_4000.csv",index=False)
