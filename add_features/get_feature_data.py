...
def analize_feature(write_path):
    import time
...
#---------------------------------------遍历action表，将所有特征都加上，然后写入文件-----------------------------------------#
    ...
    for row in action_data_array:
        ...
        if action_date == "2016-03-31":#不要3/31的数据
            continue

        if action_date == "2016-04-01":#不要4/1的数据
            continue
        ...
        comemnt_flag=0
        for comemen_date in comment_dates:
            try:
        ...
        print(user_actions,user_holiday_action,user_workday_action,views,clicks,add2cars,sales,workday_views,workday_clicks,workday_add2car,workday_sales,holiday_views,holiday_clicks,holiday_add2car,holiday_sales,weekday,is_holiday)
...
write_path = write_dir + "add_feature_train.csv"
